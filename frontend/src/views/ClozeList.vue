  ...
  <div class="min-h-screen p-3 sm:p-4 md:p-8">
    ...
    <header class="max-w-4xl mx-auto mb-6 sm:mb-8">
      <div class="flex items-center justify-between">
        ...
        <h1 class="text-xl sm:text-2xl font-bold text-mint-700">完形填空列表</h1>
        <button
          @click="goToCreate"
    ...
    <div class="max-w-4xl mx-auto">
      ...
      <!-- Empty State -->
      <div v-else-if="clozeTests.length === 0" class="text-center py-12">
        <div class="text-6xl mb-4">📝</div>
        <p class="text-mint-600 text-lg mb-4">还没有完形填空题目</p>
        <button
          @click="goToCreate"
              ...
              <div class="flex items-start justify-between">
                <div class="flex-1">
                  <div class="flex items-center gap-2 mb-2">
                    <span class="px-2 py-1 bg-mint-100 text-mint-700 rounded text-sm font-medium">
                      {{ test.word1 }}
                    </span>
...
const deleteTest = async (id) => {
  ...
  try {
    ...
    clozeTests.value = clozeTests.value.filter(t => t.id !== id)
  } catch (e) {
    error.value = '删除题目失败。'
    console.error(e)
  }
...
const goToCreate = () => {
  router.push({ name: 'cloze-test' })
}

const goToTest = (id) => {
  router.push({ name: 'cloze-test-detail', params: { id } })
}
